...
//------------------------------------------------------------------------------
extern "C" {
  MergeFC* fastcluster(int const N_, method_codes const method_, const double* D_, const int* members_) {
    MergeFC* r = NULL; // return value
    ...
    try{
      ...
      */
      ...
      const int method = INTEGER_VALUE(method_); // index-0 based;
...
//            Rf_error("'members' must have length N.");
          const t_index* const m = members_;
...
//      int* m; // return field "merge"
//      PROTECT(m = NEW_INTEGER(2*(N-1)));
      int* const merge = r->merge();
...
//------------------------------------------------------------------------------
/*
  MergeFC* fastcluster_vector(method_codes_vector const method_,
                          int const metric_,
                          double* X_,
                          double* members_,
                          double p_) {
...
//       Rf_error("'method' must be a single integer.");
      int method = INTEGER_VALUE(method_); // index-0 based;
...
//        Rf_error("'metric' must be a single integer.");
      int metric = INTEGER_VALUE(metric_); // index-0 based;
...
//      SEXP h; // return field "height"
//      PROTECT(h = NEW_NUMERIC(N-1));
      double* const height = new int[N-1];
